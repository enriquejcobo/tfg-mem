...
\item \verb@setTempResolution@. Tras enviar este comando, procedimos a solicitar de nuevo la temperatura actual, observando en dicha medida que su precisión había variado conforme se le había indicado.
...
No se han podido comprobar todas las funciones del \textbf{Acelerómetro}, puesto que la versión definitiva no llegó hasta pocos días antes del fin de este proyecto. No obstante, sí hemos podido comprobar la función \verb@getAcc@ y las auxiliares que acceden a las variables locales, necesarias para la aplicación de prueba. Es por ello que somos optimistas respecto a la operación del sensor, conociendo además que los periféricos del \ac{MCU} que usa el acelerómetro sí funcionan, ya que la interfaz \ac{I2C} ha sido testeada con éxito con el sensor de temperatura, y los \ac{CN} también han mostrado un comportamiento positivo en las pruebas desarrolladas con el \ac{PIR}.
