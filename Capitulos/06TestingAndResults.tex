...
Detallamos en primer lugar las pruebas realizadas a los sensores en su perspectiva \textit{hardware}, comenzando por el sensor de \textbf{Luminosidad}. Para verificar el correcto funcionamiento del mismo, se realizó el montaje detallado en la figura \ref{diseno:fig:lum}, y se comprobó que, para distintos niveles de iluminación (aplicándole una fuente lumínica externa o cubriendo el detector) obteníamos a la salida distintos niveles de tensión, medidos en el osciloscopio. 
...
\item Cuando acoplábamos un dispositivo, como puede ser un sensor, un actuador o un componente, realizábamos pruebas de corto, en las cuales comprobamos que las conexiones entre patas del componente eran nulas, y que había cortocircuitos entre el pad del mismo y el siguiente punto de conexión en el circuito.
\item En caso de soldar resistencias, usábamos el multímetro en posición de medir esta magnitud, de forma que comprobamos que el valor que aparecía en el medidor era similar al que debíamos obtener, con lo que verificábamos tanto una buena conexión como la inserción de la resistencia de valor adecuado.
\item Acciones similares eran seguidas con los condensadores, buscando los mismos propósitos que en el caso anterior. Para ello, poníamos el multímetro en posición de medir capacidades.
\end{itemize}
...
En este subapartado se detallan las pruebas hechas sobre el \textit{software} desarrollado, que ha sido objeto de estudio en el capítulo \ref{cap:software}. Las hemos clasificado en dos subapartados, las de la \ac{HAL} \textit{per se}, y las que se han desarrollado en combinación con los sensores y los actuadores.
...
En primer lugar probamos el \textbf{\ac{ADC}}, la cual consistió en formar un divisor de tensión por medio de resistencias conectadas a $V_{CC}$ y a GND, e introducir la tensión en distintos puntos en el \ac{ADC}, de forma que comprobáramos que la palabra obtenida era la correspondida con la siguiente fórmula: $$ADC = \left \lfloor {v_{ADC} \cdot \frac{2^n-1}{V_{CC}}} \right \rfloor = \left \lfloor v_{ADC} \cdot \frac{2^{10}-1}{3.3} \right \rfloor$$

En efecto, verificamos que las palabras en hexadecimal obtenidas cuando $v_{ADC} = 3.3$ V, $v_{ADC} = 1.65$ V y $v_{ADC} = 0$ V son, respectivamente, \verb@3FF@, \verb@1FF@ y \verb@000@, por lo que el funcionamiento del mismo es correcto.
...
En primer lugar presentamos las pruebas realizadas al sensor de \textbf{Temperatura}. De la sección \ref{software:sec:temp} recordamos el conjunto de funciones que necesita:
...
\item \verb@setTempLowPower@. La estrategia seguida fue la de comprobar que, puesto que es conocido que en este modo la temperatura no está calculándose continuamente, sino que debe solicitarse, modificamos la función \verb@getTemp@ para que no lo solicitara a pesar de estar en modo bajo consumo. Comprobamos efectivamente que, bajo estas condiciones, la lectura del sensor permanecía invariante ante cambios de temperatura. \\Una vez hecha la comprobación, se restauró la función, comprobando que de nuevo las lecturas volvían a ser coherentes.
\end{itemize}
...
No se han podido comprobar las funciones del \textbf{Acelerómetro}, puesto que, como se puso de manifiesto en el capítulo \ref{cap:diseno} no llegó la versión definitiva de la placa al cierre de esta memoria. No obstante, tenemos garantías en que las mismas funcionen, puesto que la interfaz \ac{I2C} ha sido testada con éxito con el sensor de temperatura, y los \ac{CN} también han mostrado un comportamiento positivo en las pruebas desarrolladas con el \ac{PIR}.
...
Por último, las pruebas realizadas al \textbf{Buzzer} y \textbf{LEDs} fueron el mandar el encendido y el apagado de los mismos, con las funciones ya descritas. En el caso del Buzzer, se obtuvo, como era de esperar, dos pitidos que se alternaban de forma aproximada cada segundo; y, para los LEDs, comprobamos que \verb@ledOn@ mantiene encendido el LED, \verb@ledOff@ lo apaga incondicionalmente, y \verb@ledToggle@ lo conmuta, apagándolo o encendiéndolo según el caso.
