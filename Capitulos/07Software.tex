...
A continuación detallamos los trabajos realizados en materia de desarrollar un conjunto de funciones \textit{software} responsables de intervenir sobre los sensores y los actuadores de la placa de expansión. Puesto que esta interfaz realmente abstrae los recursos \textit{hardware} implantados, se la denomina \ac{HAL}.
...
Nuestro código está integrado básicamente en dos ficheros: \verb@SensorsHAL.c@, que es el fichero maestro en el que se detallan las acciones llevadas a cabo por las funciones, y \verb@SensorsHAL.h@, en el que se encuentran las cabeceras exportadas de las mismas.
...
\begin{itemize}
\item \verb@InitSensors ()@. Su misión es la de preparar los periféricos dentro del \ac{MCU}, de forma que se habiliten los necesarios y en los pines asignados.
	\begin{itemize}
	...
	\item Por último, se configura el temporizador interno --mediante el uso de la librería \verb@timer.h@--, cuya misión se expone a continuación.
	\end{itemize}
\item \verb@IntTmp ()@. Rutina de atención a la interrupción causada por el temporizador interno TMR5, usada por los siguientes elementos:
	\begin{itemize}
	...
	\end{itemize}
\item \verb@IntCN ()@. Rutina de atención a la interrupción motivada por el cambio en alguna señal rutada hacia un \ac{CN}, como pueden ser las interrupciones 1 y 2 del acelerómetro, o la salida del sensor de presencia.
\end{itemize}
...
Aparte de estas funciones, ha sido necesario modificar en parte los siguientes archivos, pertenecientes al \textit{firmware} del \ac{CNGD}, para que la integración de la placa en el \ac{CNGD} sea posible. En este sentido, los archivos modificados han sido:
...
\item \verb@HardwareProfile.h@. Se producen las asignaciones de los pines a sus diferentes periféricos, de forma que un cambio en los mismos se simplifica únicamente modificando su etiqueta. \\
En nuestro caso, se han realizado las asignaciones necesarias en función del primer archivo, de forma que en un cambio en el mismo originaría la liberación de los recursos, permitiéndoles otro uso. Estas asignaciones son las que permiten la inicialización, modificación y consulta de los puertos.
\end{itemize}
...
\begin{itemize}
\item \verb@INT8 getTemp ()@. Esta función devuelve la medida de la temperatura en forma de un entero, si bien éste representa un número real de coma fija, en el que el primer byte representa la parte entera y el segundo la parte decimal.
...
\item \verb@setTempAlert (int reg, INT8 alert)@. Modifica los registros correspondientes a los umbrales superior (\verb@MAX_HIGH@) e inferior (\verb@MAX_LOW@), poniendo el valor de temperatura \verb@alert@ en los mismos. \verb@alert@ debe redondearse al entero más próximo.
...
A continuación mostramos aquellas funciones que, aunque no se encuentran exportadas, cumplen la misión de simplificar el código, evitando duplicidades si dentro de dos funciones distintas se requería hacer la misma acción. En el caso del sensor de temperatura, serán las que implementen principalmente las funciones que proporciona el fabricante en \verb@i2c.h@ para la comunicación por medio de este protocolo.

\begin{itemize}
\item \verb@setTempConf ()@. Realiza la escritura en el registro de configuración. Se encuentra exportada al ser requerida por todas las funciones que realizan cambios sobre el mismo, como pueden ser \verb@setTempResolution ()@ o \verb@getTempLowPower ()@.
\item \verb@setTempRegister (UINT8 reg)@. Abre la comunicación \ac{I2C} haciendo las peticiones necesarias para leer el registro que se le pasa por parámetro. Es la función que la llama la responsable de obtener el dato y de cerrar la comunicación.
\item \verb@INT8 getTempConf ()@. Función auxiliar que permite comprobar que el estado del registro interno de configuración coincide con la copia de él que el programa dispone. De esta manera garantizamos la concordancia entre ambos.
\end{itemize}
...
Para consultar el valor de luminosidad, basta con llamar a \verb@int getLum ()@, el cual solicita realizar la medida al \ac{ADC}. Para obtener un dato actual, desechando valores anteriores, esta petición se realiza 10 veces. Esta función devuelve un entero que representa la tensión.
...
La comunicación infrarroja se implementa bajo diversos estándares, puesto que cada fabricante puede definir su propio protocolo de comunicaciones con su equipo. Como muestra de ello se presentan, en la figura \ref{software:fig:tramasvarias}, las tramas procedentes de mandos a distancia de fabricantes distintos.
...
No obstante, estos estándares sí unifican el hecho de que para este tipo de comunicaciones se usan modulaciones basadas en el ancho de pulsos sobre portadoras de 34 kHz.

Esto nos lleva, en primer lugar, a elegir el electrodoméstico que queramos controlar, de forma que podamos implementar su protocolo. En nuestro caso, decidimos controlar el aire acondicionado, debido a que la placa dispone de sensor de temperatura y puede conocer, por tanto, el valor de la misma en tiempo real.

Evaluando trabajos previos \cite{javipfc}, obtenemos que la información en las tramas se organiza de la siguiente manera: En primer lugar en 2 bytes se codifica el dispositivo, seguido de un byte de protección frente a errores, el comando a mandar --con tamaño variable-- y, por último, su código redundante.

Su implementación física se detalla en la figura ().
...
\begin{itemize}
\item \verb@protocoloAA()@. Incluye la máquina de estados correspondientes a la trama presentada en (). Su misión es que, en cada interrupción del temporizador, se ponga en la salida del \ac{GPIO} el valor que corresponda, según la posición en la que nos encontremos.
...
La función que permite añadir comandos a la cola de transmisión del aire acondicionado se denomina \verb@sendAA (int modo, int estado)@, en la que los argumentos indican si se quiere entrar en modo verano (\verb@AA_Summer@) o invierno (\verb@AA_Winter@); o bien si se quiere encender el aparato (\verb@AA_On@) o apagarlo (\verb@AA_Off@).
