...
Análogamente, procedemos a presentar las funciones que requieren los actuadores incorporados, bajo las mismas condiciones enunciadas en las subsecciones anteriores.
...
La implementación llevada a cabo en esta \ac{HAL} ha sido la de usar un \textit{buffer}, puesto que previmos que en la implementación final del sistema se tendría la necesidad de enviar varios comandos seguidos. El tamaño del mismo puede contrarle por la constante \verb@BUFFER_MAX@. Además, con este método conseguimos que la transmisión por el puerto \ac{IR} no sea bloqueante del programa principal, puesto que la transmisíón entera dura 67.5 ms, y el encadenamiento de varias puede originar un paro significativo en la ejecución.

Como resultado de requerir una modulación, será necesario emplear de nuevo el temporizador interno, con la frecuencia suficiente para ser capaz de obtener una señal pulsada de 38 kHz. Como consecuencia, la máquina de estados que gobierne la transmisión y el \textit{buffer} tendrá que llamarse desde la rutina de atención a la interrupción, si bien es muy ligera desde el punto de vista de tiempo de ejecución. Esto además posibilita el hecho de que se pueda seguir ejecutanto el programa principal entre activaciones de la interrupción.

La función que permite añadir comandos a la cola de transmisión se denomina \verb@sendIR (int address, int command)@, en la que los argumentos indican tanto la dirección \ac{IR} de la máquina como el comando a transmitir. Sugerimos que, de cara a una implementación final, se añadan como constantes las direcciones y de los comandos, de forma que sean fácilmente referenciables.
...
Para este actuador volveremos a requerir la interrupción periódica, la cual, como hemos visto en la sección \ref{software:sec:ir}, es capaz de soportar las frecuencias que se precisan.
...
Como se expuso en la sección \ref{diseno:sec:leds}, se han introducido dos \ac{LED} que permitan mostrar distintas señales, en función del contexto y del sistema final. Las funciones que producen su modificación son las que siguen:
...
\verb@sensorLed@ es un tipo definido que consta de los valores \verb@GREEN@, \verb@RED@ los cuales identifican al \ac{LED} por su color, y \verb@BOTH@, que refiere que se quiere actuar sobre ambos.
...
No cabe olvidar que la meta del trabajo reside en incluir nuestra \textit{shield} dentro de un nodo de una \ac{CWSN}, por lo también es necesario que las placas implantadas en distintos nodos sean capaces de comunicarse entre ellas.

En este sentido, se desarrolla una aplicación que consta de los siguientes pasos:

\begin{enumerate}
\item Se llevan a cabo las rutinas de inicialización del sistema, tanto del \ac{CNGD} como de la placa de Sensores y Actuadores.
\item Se establecen las comunicaciones, siguiendo el protocolo \ac{P2P} Unicast, y se deciden los canales por los que se realizará la comunicación.
