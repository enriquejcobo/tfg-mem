...
\begin{resumen}
En este capítulo encontramos una primera aproximación de las necesidades que requiere la placa de expansión, en cuanto a módulos y funcionalidades se refiere. Asimismo, se presentarán, para cada uno de estos módulos, una justificación de su uso y sus puntos más destacables.
\end{resumen}
...
%-------------------------------------------------------------------
Como vimos en el apartado \ref{intro:sec:antecedentes}, la placa de expansión se realizará para su anexión a la plataforma \ac{CNGD} \cite{aguspfc}, por lo que hacemos una breve introducción a ella. La figura \ref{previo:fig:cngd} muestra detalladamente este nodo.
...
Una de las principales ventajas de este diseño es la cabecera de expansión, la cual permite ampliar el diseño de forma modular y totalmente escalable. Será en la misma donde introduciremos nuestra placa, por lo que le hacemos un estudio más exhaustivo. Se compone de 40 pines, cada uno pudiendo estar multiplexado con diversas funciones, en el que se permite el acceso a los periféricos del microcontrolador. En la tabla \ref{previo:tab:headerspinout} encontramos una relación entre el pin del header, el pin del microcontrolador y las funciones que puede realizar.
...
Finalmente, el \ac{CNGD} es programado por medio del módulo de programación PGE. Esta misma interfaz es usada para la depuración In-Circuit del sistema. Esto será util para comprobar la ejecución correcta del código desarrollado, por lo que se ampliará en el capítulo \ref{cap:software}.
...
%-------------------------------------------------------------------
Los sensores van a ser la entrada de información a nuestro sistema, ya que por medio de sus mediciones conoceremos los parámetros del entorno, de forma que podamos actuar sobre él. Es por ello que debemos escogerlos adecuadamente, por lo que definimos el procedimiento que detallamos a continuación:
...
\item \emph{Evaluar las \ac{DS}}. Las \ac{DS} son las hojas de características que proporcionan los fabricantes en las que se detallan todas las propiedades que definen su producto. De esta evaluación obtenemos la verificación de que cumplen las constantes definidas.
...
\item \emph{Salida}. La podemos clasificar en dos tipos: 
	...
	\item Digital, en el cual la medición se representa por una palabra binaria, y que se conecta al microcontrolador, generalmente, por buses serie, como pueden ser \ac{SPI} o \ac{I2C}.
	\end{itemize}
No imponemos ningún filtro a priori, puesto que el \ac{CNGD} dispone de entradas tanto digitales como analógicas.
\item \emph{Consumo}. No debemos olvidar que el sistema, al estar alimentado con baterías, requiere un estricto control sobre el gasto energético. Este requisito fue considerado a la hora del diseño del \ac{CNGD}, permitiendo la desconexión o la suspensión de los dispositivos que no se encuentran en uso, y por tanto será también objeto de evaluación.\newline Se buscará, de entre toda la gama de sensores, aquellos cuyas especificaciones de consumo sean menores, o bien que puedan ser sometidos a estados de bajo consumo.
...
Texas Instr. & TMP101 & [-55 ºC, 125 ºC] & $\pm$2 ºC & Digital & 2.56 \euro & \cite{tmp101} \\ \hline
Analog Devices & AD7814 & [-55 ºC, 125 ºC] & $\pm$2 ºC & Digital & 2.71 \euro & \cite{ad7814} \\ \hline
Microchip & MCP9800 & [-55 ºC, 125 ºC] & $\pm$0.5 ºC & Digital & 0.98 \euro & \cite{mcp9800} \\ \hline
\end{tabular}
}
...
%-------------------------------------------------------------------
Definimos un acelerómetro como un sensor que nos permite medir las aceleraciones asociadas con el fenómeno de peso experimentado por una masa de prueba. Podemos encontrar varios tipos, a saber:
...
A priori, puede parecer que introducir un sensor de este tipo es extraño para la obtención de parámetros de un entorno. No obstante, el uso de este tipo de sensores está creciendo en multitud de aplicaciones, por la versatilidad que le caracteriza. Tanto es así que puede ser usado, por ejemplo, para detectar la posición de la placa en el plano tridimensional, o para detectar vibraciones, bien generadas por la acción humana (como pisadas en el suelo o vehículos pasando por un puente) o por la naturaleza (como los terremotos).
...
\item \emph{Encapsulado}. De todos los sensores evaluados es el más grande, ya que el conjunto sensor y lente hace que sea así. Esto nos puede generar problema a la hora de decidir su posición en la \ac{PCB}.\newline Aunque no podamos poner ninguna restricción a priori, sí que será un parámetro a comparar entre todos los candidatos que encontremos, por lo que consideraremos si es \ac{SMD} o \ac{TH}.
\end{itemize}
...
\hline
Fabricante & Modelo & Lente & Distancia & Encap. & Salida & Precio\footnote{Incluye el precio por sensor y lente, en caso de que la última sea externa.} & \ac{DS} \\ \hline \hline
Murata & E710ST0 & Externa & [3 m, 6 m] & SMD & Analógica & 5.61 \euro & \cite{e710st0} \\ \hline
Panasonic & EKMC160 & Integrada & 5 m & TH & Digital & 15.49 \euro & \cite{ekmc} \\ \hline
Panasonic & AMN33112J & Integrada & 3 m & TH & Digital & 11.91 \euro & \cite{amn} \\ \hline
\end{tabular}
}
...
La inserción de este actuador en esta placa está más que justificada, ya que son incontables los dispositivos que pueden controlarse hoy en día mediante un mando a distancia por \ac{IR}, como pueden ser la televisión, equipos de música, o, en relación con los actuadores expuestos en \ref{previo:sec:sensores}, la climatización y las persianas, entre otros.
...
%-------------------------------------------------------------------
Un \ac{LED} es un material semiconductor en el cual se crean fotones a partir de electrones, en un proceso denominado generación. El color de emisión, determinado por la longitud de onda, dependerá directamente del material que lo constituya.
...
%-------------------------------------------------------------------
Un \ac{GPIO} es un puerto, generalmente de un microcontrolador, que puede actuar como entrada o salida digital, y que se puede usar cualquier tipo de aplicación.
