...
\begin{resumen}
En este capítulo encontramos, en primer lugar, una revisión de la plataforma cNGD, base de la placa de expansión. Seguidamente, se expone una primera aproximación de las necesidades y requisitos de la misma, en cuanto a módulos y funcionalidades se refiere. Asimismo, se presentarán, para cada uno de estos módulos, una justificación de su uso y sus puntos más destacables.
\end{resumen}
...
Comenzamos el trabajo describiendo brevemente el \ac{CNGD}, plataforma para redes de sensores inalámbicas cognitivas, puesto que será sobre esta donde se acoplará nuestra \textit{shield}, lo que hace indispensable conocer sus características. La figura \ref{previo:fig:cngd} muestra detalladamente este nodo.
...
Una de las principales ventajas de este diseño es la cabecera de expansión --también denominada \textit{header}--, la cual permite ampliar el diseño de forma modular y totalmente escalable. Será en la misma donde insertaremos nuestra placa, por lo que le hacemos un estudio más exhaustivo. Se compone de 40 pines, cada uno pudiendo estar multiplexado con diversas funciones, en el que se permite el acceso a los periféricos del microcontrolador. En la tabla \ref{previo:tab:headerspinout} encontramos una relación entre el pin del \textit{header}, el pin del microcontrolador y las funciones que puede realizar.
...
A continuación comenzamos a desarrollar los sensores que incluiremos en la placa de expansión. Estos van a ser la entrada de información a nuestro sistema, ya que por medio de sus mediciones conoceremos los parámetros del entorno. Dada su relevancia debemos escogerlos adecuadamente, por lo que definimos el procedimiento que detallamos a continuación:
...
\item \emph{Buscar sensores comerciales}. Tomando como base los resultados de la fase anterior, y usando las páginas webs de los principales proveedores (como Farnell \cite{farnell}, RS \cite{rs}, Digikey \cite{digikey} o SparkFun \cite{sparkfun}), realizaremos la búsqueda.
\item \emph{Estudiar las hojas de características}, también conocidas como \ac{DS}. Estas son proporcionadas por los fabricantes y en ellas se detallan todas las propiedades que definen su producto. De esta evaluación obtenemos la verificación de que cumplen las constantes definidas.
...
\hline
Fabricante & Modelo & Rango de Medida & Precisión\footnote{Típica en el margen que incluye a 25 ºC.} & Consumo\footnote{Típico durante el modo de funcionamiento} & Precio & \ac{DS} \\ \hline \hline
Analog Devices & TMP36 & [-40 ºC, 125 ºC] & $\pm$1 ºC & 50 $\mu$A & 1.61 \euro & \cite{tmp36} \\ \hline
Microchip & MCP9701A & [-40 ºC, 125 ºC] & $\pm$2 ºC & 6 $\mu$A & 0.24 \euro & \cite{mcp9701} \\ \hline
Texas Instr. & LM20B & [-55 ºC, 130 ºC] & $\pm$2 ºC & 10 $\mu$A & 1.07 \euro & \cite{lm20} \\ \hline
Texas Instr. & TMP101 & [-55 ºC, 125 ºC] & $\pm$2 ºC & 45 $\mu$A & 2.56 \euro & \cite{tmp101} \\ \hline
Analog Devices & AD7814 & [-55 ºC, 125 ºC] & $\pm$2 ºC & 250 $\mu$A & 2.71 \euro & \cite{ad7814} \\ \hline
Microchip & MCP9800 & [-55 ºC, 125 ºC] & $\pm$0.5 ºC & 200 $\mu$A & 0.98 \euro & \cite{mcp9800} \\ \hline
\end{tabular}
}
...
\begin{tabular}{|c|c|c|c|c|c|c|}
\hline
Fabricante & Modelo & Fondo de escala & Ejes & Consumo\footnote{Típico en estado de medición contínuo.} & Precio & \ac{DS} \\ \hline \hline
...
\begin{tabular}{|c|c|c|c|c|c|c|}
\hline
Fabricante & Modelo & Lente & Distancia & Consumo\footnote{Típico en detección.} & Precio\footnote{Incluye el precio por sensor y lente, en caso de que la última sea externa.} & \ac{DS} \\ \hline \hline
...
%-------------------------------------------------------------------
Un sensor de luminosidad es aquel que permite determinar la cantidad de luz que se encuentra en el espacio de medida del mismo. Podemos encontrar principalmente dos tipos: las fotorresistencias, conocidas por sus siglas inglesas \ac{LDR}, las cuales son unas resistencias cuyo valor depende de la luz, y los fotodiodos, que ofrecen a la salida una corriente que es proporcional al nivel de iluminación que encuentren. Serán estos últimos sobre los que centraremos nuestra búsqueda al ser más comunes en aplicaciones de propósitc general.
...
\hline
Fabricante & Modelo & Pico en $\lambda$ & Oscuridad & Iluminación\footnote{Típico bajo 100 luxes.} & Salida & Precio & \ac{DS} \\ \hline \hline
...
\label{previo:sec:ir}
%-------------------------------------------------------------------
Un emisor \ac{IR} no es más que un \ac{LED} cuyo espectro de emisión de luz se centra en estas longitudes de ondas, típicamente en el entorno de los 630 nm. Esta señal es modulada para la transmisión digital de comandos, propios para cada dispositivo.

La inserción de este actuador en la placa está más que justificada, ya que son incontables los dispositivos que pueden controlarse hoy en día mediante un mando a distancia por \ac{IR}, como pueden ser la televisión, equipos de música, o, en relación con los sensores expuestos en \ref{previo:sec:sensores}, la climatización y las persianas, entre otros.
...
El propósito de este actuador no es otro que el de avisar a los humanos acerca de las condiciones ambientales, generalmente cuando suponen la existencia de un peligro potencial, para que sean éstos los que, en último término, lleven a cabo la actuación.
...
Como vimos en \ref{previo:sec:cngd}, nuestra plataforma, por medio de sus \textit{header}s, tiene accesible varios puertos \ac{GPIO}, por lo que es posible, a partir de ellos, controlar cualquier dispositivo externo cuya señal de control sea digital, como puede ser un relé.
